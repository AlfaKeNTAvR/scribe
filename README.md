...
| `check --base <ref>` | The CI merge gate: fails a pull request that introduces or depends on an unreviewed record superseding a ratified one, or that breaks the append-only or immutability rules. Deleting a record that was present at the base ref also fails the check (`record_deleted`); retire it with `expired` or `backtracked` instead. |
