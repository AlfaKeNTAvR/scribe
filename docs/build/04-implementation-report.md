...
- Live plugin test (`docs/build/pipeline/step5_live_test.sh`): `claude -p --plugin-dir /home/alfakentavr/scribe --model sonnet` asked to edit `src/scribe/index.py`. Result: the model reported `INJECTION SEEN, alias D-260908-unreviewed-may-supersede-ratified`; `.claude/scribe/state.json` gained the session with its prompt id (SessionStart and UserPromptSubmit fired), `gate-log.jsonl` gained a `stop_reconcile` line (Stop fired), `hook-errors.log` stayed absent. This settles the exec-form hooks.json shape and the `additionalContext` output live, not only in tests. The edit itself was refused by the headless run's own permission guard ("sensitive file"), unrelated to scribe.
- Docs check via the claude-code-guide agent (2026-09-08 evening): U1 resolved, `$ARGUMENTS`, `${CLAUDE_SESSION_ID}` and `${CLAUDE_PLUGIN_ROOT}` are substituted inside a skill's inline command before it runs (skills docs). U2 stays not documented. `Edit(/path)` deny rules are project-relative and cover Edit; Write rules are not consulted, so the deny rule on `RATIFICATIONS.jsonl` is the right form. U3 (Windows) untested.
- Cosmetic: the T6 acceptance run left a fixture session (`session_fixture_0002`) in this repository's `.claude/scribe/state.json`; the 7-day prune removes it.
